...
bool TGAImage::read_tga_file(const std::string filename) {
        ...
        if (w <= 0 || h <= 0 ||
            (bpp != GRAYSCALE && bpp != RGB && bpp != RGBA)) {
        ...
        do {
                ...
                if (chunkheader < 128) {
                        ...
                        for (int i = 0; i < chunkheader; i++) {
                                in.read(
                                    reinterpret_cast<char *>(colorbuffer.bgra),
                                    bpp);
                                if (!in.good()) {
                                        std::cerr << "an error occured while "
                                                     "reading the header\n";
                                        return false;
                                }
                                for (int t = 0; t < bpp; t++)
                        ...
                        chunkheader -= 127;
                        in.read(reinterpret_cast<char *>(colorbuffer.bgra),
                                bpp);
                        if (!in.good()) {
                                std::cerr << "an error occured while reading "
                                             "the header\n";
                                return false;
                        }
                        for (int i = 0; i < chunkheader; i++) {
...
bool TGAImage::write_tga_file(const std::string filename, const bool vflip,
        ...
        header.height = h;
        header.datatypecode =
            (bpp == GRAYSCALE ? (rle ? 11 : 3) : (rle ? 10 : 2));
